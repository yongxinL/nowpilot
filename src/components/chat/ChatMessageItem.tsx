...
import { StructuredErrorCard, parseToStructuredError } from './StructuredErrorCard';
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Activity Block (with model passed as read-only diagnostic metadata) */}
        <ThoughtProcessBlock
          thoughtText={
            msg.thoughtProcess ||
        ...
        {/* Structured Error Card if provider error occurred */}
        {!msg.isThinking &&
        !msg.isStreaming &&
        currentContent &&
        (currentContent.toLowerCase().includes('error generating response') ||
          currentContent.toLowerCase().includes('provider returned http') ||
          (currentContent.toLowerCase().includes('api key') &&
            currentContent.toLowerCase().includes('required'))) ? (
          <StructuredErrorCard
            error={parseToStructuredError(currentContent)}
            onRetry={() => onRegenerate(msg.id)}
            onOpenSettings={() => {
              if (onOpenOptions) {
                onOpenOptions();
              } else {
                window.dispatchEvent(
                  new CustomEvent('nowpilot:open-options', { detail: { tab: 'general' } })
                );
              }
            }}
          />
