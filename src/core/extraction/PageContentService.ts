...
/**
 * Page extraction orchestrator (D-07, D-10, D-11, D-13, D-16, D-18).
 *
 ...
 * typed PageContext construction. Operational failures are returned as
 * `{ ok: false, error: ExtractionError }` — never thrown (D-11).
 */
...
export class PageContentService {
   ...
   * invalidation). Call from the side panel / full app entry point at
   * startup — NOT from the service worker or content script.
   */
  init(): void {
  ...
  /** Explicit cache invalidation API (D-13); the next extract is fresh. */
  reExtract(tabId: number): void {
    this.pageContentCache.invalidate(tabId);
  }

  /**
   * D-03 wiring: SPA_NAVIGATION events from the content script invalidate the
   * cache for the sending tab when the announced URL differs from the cached
   * URL. Same-URL navigations keep the cache hot.
   */
