...
must_haves:
  ...
  flagged_assumptions:
    ...
    - "A5 [research, ACTIVE — user deviation]: defuddle is installed at 0.19.2 (USER DEVIATION from the ^0.6/0.6.6 pin, approved at the 04a-01 blocking-human gate) — at 0.19.2 `useAsync` EXISTS and DEFAULTS TO TRUE (README documents third-party fetch fallback), so DefuddleStrategy MUST construct with `useAsync: false` explicitly to preserve the R-10 zero-network-call privacy guarantee; the sync parse() is used exclusively; the guard comment in DefuddleStrategy travels with any future upgrade. Verified at runtime: `markdown`/`separateMarkdown` options remain no-ops in the 0.19.2 browser bundle (toMarkdown wired only in dist/node.js) — Pitfall 1 holds, turndown stays the single converter (D-4a-16 heading chunking depends on markdown shape)."
...
| T-4a-05 | Information Disclosure | defuddle network exfiltration | high | mitigate | Installed at 0.19.2 (user deviation) — `useAsync` DEFAULTS TO TRUE, so DefuddleStrategy constructs with `useAsync: false` explicitly + uses the sync parse() only (zero fetches — verified in dist source); the guard comment mandates `useAsync: false` for any future upgrade (A5) |
