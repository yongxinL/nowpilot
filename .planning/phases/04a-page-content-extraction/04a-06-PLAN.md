...
must_haves:
  ...
  prohibitions:
    - flag: "flagged-unverified"
      statement: "The D-02 redaction heuristic MUST NOT be narrowed for secret-bearing field names — the NON_PASSWORD_NAME_PATTERN allowlist (passenger|passport|compass|bypass) must never grow to cover passcode/pin/otp/secret/passphrase/passage-class names; a captured password value entering the AI pipeline is unrecoverable"
...
<task type="auto">
  ...
  <action>
    ...
    In src/core/extraction/PageContentService.ts extract() (lines 82-101): pass mode through both cache calls — pageContentCache.get(tabId, mode, url) at line 83 and pageContentCache.set(tabId, mode, url, result) at line 95. Change the in-flight key at line 86 from `${tabId}:${url}:${mode}` to `${tabId}:${mode}:${url}` to match the cache key format (consistent ordering: tabId, then mode, then url) — both keys serve different Maps but share the same composite components; keeping them in the same order aids readability. Do NOT change the SPA handler or other invalidation call sites in this task — Task 3 restores index/cache consistency there. Keep the module-level pageContentCache singleton export unchanged.
...
<task type="auto">
  ...
  <action>
    ...
    1. Keep `const PASSWORD_NAME_PATTERN = /pass(word|wd)?|pwd/i;` unchanged (D-02 coverage: user_pwd, user_passwd, db_pwd, login_password, confirmPassword, user_pass, passphrase, passcode all still match).
     2. Add next to it: `const NON_PASSWORD_NAME_PATTERN = /passenger|passport|compass|bypass/i;` with a comment listing the field classes each term protects (travel/airline forms, bearings, auth-bypass flows) and citing WR-03. Note: `passage` is deliberately NOT in the allowlist — `passage`-prefixed names (passage_number, boarding_passage) don't carry credential risk; they're a travel/content domain and covered by the deliberate omission of false-negative space.
    ...
    4. Replace BOTH name-heuristic usages with isPasswordFieldName: the nameMatchedInputs filter (lines 68-71) and the clone-redaction loop (lines 87-92). No other code changes.
     5. Documented D-02 decision (put it in the helper's comment): `passage` and `passcode` are deliberately NOT in the allowlist — `passcode` fields hold PIN-like secrets (D-02 err-on-false-positive says omit rather than capture, RESEARCH Pitfall 4); `passage`-prefixed names (passage_number, boarding_passage) are travel/content fields that share no credential risk. The review's suggested allowlist (passenger|passport|compass|bypass|passage|passcode) has been narrowed to 4 terms: the `passage` and `passcode` exclusions are explicit, tested, and must never be added back.
    ...
    Tests in tests/core/content/DomSerializer.test.ts:
     ...
     3. New test 'isPasswordFieldName returns false for passage-prefixed names (WR-03 regression)': assert isPasswordFieldName('passage_number') === false AND isPasswordFieldName('boarding_passage') === false (passage is a deliberate non-credential exclusion — ensures refactoring never re-introduces it).
     4. The 3 existing name-heuristic tests (DomSerializer user_pwd, PageContextBridge user_passwd and db_pwd) require no edits — none of those names matches the allowlist — and must stay green.
  </action>
  <verify>
  ...
  <done>
    passenger/passport/compass/bypass field values are retained in serialized HTML (new FP test green); passcode values stay redacted (new test green); passage-prefixed names return false from isPasswordFieldName (regression test green); user_pwd/user_passwd/db_pwd redaction tests stay green; DomSerializer exports isPasswordFieldName for Task 5.
  </done>
</task>
...
| T-04a-06-02 | Information Disclosure | DomSerializer name-heuristic allowlist (WR-03) | high | mitigate | Task 4: contains-match retained; only the 4 documented innocuous substrings excluded via NON_PASSWORD_NAME_PATTERN inside exported isPasswordFieldName; 'passcode' and 'passage' deliberately kept redacted (D-02 err-on-false-positive); FP tests assert passenger/passport/compass/bypass values retained while secret-bearing and travel-field names stay redacted |
...
4. WR-03 closed: passenger/passport/compass/bypass values retained in serialized HTML; passcode and passage-prefixed values still redacted; isPasswordFieldName('passage_number') and isPasswordFieldName('boarding_passage') both false; user_pwd/user_passwd/db_pwd/login_password/confirmPassword values still redacted; the 3 existing name-heuristic tests stay green
