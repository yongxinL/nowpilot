...
must_haves:
  truths:
    ...
    - "ContextOptimizer.optimize() inserts the trust stage between input and packSections (D-4b-04/08/09): pageContext → contextFeed.pageToContextItems → injectionScreener.classifyInjection (quarantine decision → excluded.set(item.id, { reason: 'prompt_injection' })) → applyTrustPolicy (authority strip + wrap, O.3) → applySourceGates (D-4b-08, writes excluded.set(item.id, { reason: 'trust_disabled' }) into the SAME map) — ONE structured `excluded: Map<string, { reason: TrustOmitReason }>` (TrustOmitReason from 04b-01 harness.ts) accumulates both producers, no conversion — → buildReceipt(items, { excluded }, kindStable) (contextText + ContextReceiptEntry[] + CTX-06 counters) → contextText flows into buildPackInput/ContextPackInput.contextText (the existing 'context' section slot, stable:false, TASK_KINDS)."
...
<task type="auto">
  ...
  <action>
    ...
    - Pipeline order (D-4b-04/05/08): items = pageToContextItems(pageContext) → maintain ONE structured `excluded: Map<string, { reason: TrustOmitReason }>` for the whole stage (TrustOmitReason from 04b-01 harness.ts — `TrustOmitReasonSchema = z.enum(['prompt_injection','trust_disabled'])`; buildReceipt consumes this exact shape so there is NO conversion step) → for each item: classifyInjection(text) → hit ? excluded.set(item.id, { reason: 'prompt_injection' }) (quarantine — D-4b-06; item stays a ContextItem, never a section) : proceed → applyTrustPolicy(items) (authority strip + wrap — O.3) → applySourceGates(policyItems, input.trustPrefs ?? DEFAULT_TRUST_PREFS) returns its OWN `{ included, excluded: Map<string, { reason: TrustOmitReason }> }` (D-4b-08, values { reason: 'trust_disabled' }) whose excluded entries MERGE into the same stage-level map (identical shape — plain spread, no transformation) → buildReceipt(policyItems, { excluded }, kindStable, screened, quarantined) where kindStable = (kind) => CACHED_KINDS semantics — page items map to the 'context' section which is stable:false, so return false for context-kind (use the existing ProviderRouter CACHED_KINDS import? NO — keep the optimizer dependency-light: define `kindStable = (kind) => kind === 'memory'`-style inline with a comment naming CACHED_KINDS as the single source (F-5); the context section is always stable:false so page-kind → false).
