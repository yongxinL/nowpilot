...
# Operator disposition (2026-09-24, plan 02-14 Task 4): the CR-01 metadata-hold residual is
# acknowledged and scheduled as WINDOWS #26 (owner: the earliest approved gap-closure or
# storage-hardening plan that may modify the store write guard, defaulting to the Phase 19
# release-hardening plan; expiry before v0.2 release acceptance; the Phase 19 release gate
# must fail while it remains unimplemented or lacks an explicit later operator decision) and
# the CR-02 mirror-persists-on-promotion residual is accepted as designed under D2-34
# (status: accepted; severity: low; no follow-up; not waived, not a defect). CR-01 is not
# fixed and not waived; WINDOWS #5, #7, #8, #25 and #26 all stay open.
status: draft
