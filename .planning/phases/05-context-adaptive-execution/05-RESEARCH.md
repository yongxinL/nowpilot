...
All six questions were locked as decisions in 05-01-PLAN.md: Q1 minimal shapes live in `src/core/context/types.ts` (discretion-sanctioned sibling) · Q2 budget→section mapping locked (history lives inside [CONTEXT]; manifest has no 'history' kind) · Q3 `assemble()` emits the sourced five kinds; SYSTEM/TASK recorded as by-design omission records (truncated: true, 0 tokens) · Q4 `CONTEXT_TOO_LARGE` returned as the discriminated union `{ ok:false, code, message, totalTokens, inputBudget, minimalMode, truncatedSources }` (never a throw; not added to StreamErrorCodeSchema) · Q5 §1.4 `contextTierCaps` helper SHIPPED unwired · Q6 `workspaceId`/`activeSurface` REQUIRED with no defaults.
