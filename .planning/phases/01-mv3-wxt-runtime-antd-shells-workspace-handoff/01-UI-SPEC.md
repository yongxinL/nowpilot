...
| E2 | **populated** | CommandPalette — Flow 10 base set (5 commands) | Renders the surface's 4-command set as the default state (CONTEXT D-08): Side Panel = open-standalone-view, open-options, toggle-theme, reload-extension; Standalone view = focus-side-panel (Phase-1 stub per D-06), open-options, toggle-theme, reload-extension. (The Flow 10 base set is 5 commands total, but each surface registers only its applicable 4 — no surface ever shows all 5.) Each command row = name (16 px semibold) + description (12 px secondary) + category (12 px tertiary, right-aligned). Active row uses colorPrimaryBg highlight. |
...
| Side Panel post-handoff mirror banner | Height 32 px (reusing the DESIGN_SYSTEM §8.1 28 px status-bar text pattern with 2 px top/bottom breathing room), full-width, colorPrimaryBg background (subtle, NOT colorWarning/colorError — the banner is informational), hairline colorBorder top + bottom. Left = "Switched to Standalone." (caption 12 px `colorTextBase`). Right = "Refocus here" link (12 px `colorPrimary`, underlined — normal action link, **NOT destructive**). Banner slides down 150 ms on appear (DESIGN_SYSTEM §11 motion table). | **Copy ("Switched to Standalone.") → refocus action ("Refocus here" link).** Banner is a status announcement; eye lands on copy first, then the action link. |
