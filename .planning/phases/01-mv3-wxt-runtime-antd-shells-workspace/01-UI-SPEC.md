...
> **Canonical Sync (2026-08-07) — applied after this contract's original approval to align it with later spec/design decisions:**
> 1. **Composer toolbar** — removed the **Snip/Screenshot** icon (screen capture now lives inside the **Attach** popover); the toolbar is model selector **+ Agent-mode toggle** (left) · Attach · History · New chat (right, icon-only).
> 2. **Agent is a *mode* of Chat, not a page** (spec §17.8) — removed "Agent" as a Standalone nav/page surface; its steps show in the Chat Thought-process expander.
> 3. **Standalone Sider nav** — Main group **Chat · Note · Write · Tools** + a separated **Add-ons group (TeamGQM)**; **Options moved to the Sider footer** (no longer a nav item).
> 4. **Sider collapse width** corrected to **240 px → 72 px** (was 56 px); the 1024 px viewport `Alert` is a separate width guard.
> 5. **Empty states** — replaced the Agent page empty-state with **Write** and **Tools** empty-state copy.
> 6. **Typography** — Design System row now reads **"Inter 600"** (was 600–700), matching the 2-weight cap.
>
> ⚠️ **Scope note for the planner:** items 1–6 keep this contract consistent with the current `PRODUCT_SPEC_v0_1.md` / `DESIGN_SYSTEM.md`. Whether **Write/Tools** surface *shells* are in Phase 1 build scope (vs. later phases) is governed by `01-CONTEXT.md`; if Phase 1 ships only Chat/Notes shells, treat the Write/Tools nav entries + empty states here as **registered-but-deferred** placeholders (same pattern as the other not-yet-built surfaces). Re-run `gsd-ui-checker` after this sync to re-stamp the 6-dimension sign-off.
...
| Font | UI/body: **Inter** with system fallback stack `system-ui, -apple-system, "Segoe UI", Roboto, sans-serif`. Display/wordmark: **Inter** 600, tight tracking. Mono/code/macros/IDs: **JetBrains Mono** with fallback `ui-monospace, SFMono, Menlo, monospace`. Single UI family; weight carries hierarchy (DESIGN_SYSTEM.md §5). |
...
| Empty state — Write (Standalone skeleton; full surface later) | **Draft something new, or pick a template.** |
| Empty state — Tools (Standalone skeleton; full surface later) | **No tools connected yet. Add an MCP server or enable a built-in tool.** |
...
3. **Composer block** — toolbar (model selector + Agent-mode toggle left; Attach / History / New chat right, icon-only — no Snip icon; screen capture lives inside the Attach popover) → input → status bar (provider left, Help / Feedback right). Last scan target.
...
2. **Active page content** — the page skeleton for the active nav surface: **Chat · Note · Write · Tools** (main group) or **TeamGQM** (add-ons group). Agent is a *mode* of Chat (composer toggle), not a page; Options opens from the Sider footer, not a nav item.
...
Skeletons over spinners (DESIGN_SYSTEM.md §9 / spec §17.4). The Chat / Note / Write / Tools page skeletons each use an AntD `Skeleton` block (avatar + 2–3 paragraph rows) on the same 8 / 12 / 16 px rhythm as the populated state — no layout shift when data arrives.
...
| empty | Standalone Sider (E4) | ✅ covered | E4 always renders the four main nav entries (Chat/Note/Write/Tools) plus the TeamGQM add-on entry; Options sits in the footer, not the nav. There is no zero-items state in Phase 1; the active-item pill is the only state-driven change. |
| empty | Standalone page content (E5) | ✅ covered | E5 shows each page's documented empty-state copy — Notes "Notes live here once you save your first one." with a "New note" CTA; Write "Draft something new, or pick a template."; Tools "No tools connected yet…"; the empty states are always visible behind any Phase 1 placeholder. (Agent is a mode of Chat, so it has no separate page empty-state — its empty hint is Chat's.) |
...
| populated | Standalone Sider (E4) | ✅ covered | E4 renders the populated state — a **Main group** (Chat/Note/Write/Tools) and a separated **Add-ons group** (TeamGQM); active item highlighted with `colorPrimaryBg` pill + `colorPrimary` icon/text; footer with Cmd+K hint, settings gear, profile avatar (Options opens from the footer). |
...
| partial | Standalone Sider (E4) | ✅ covered | E4 degrades gracefully — the Sider always renders the Main group (Chat/Note/Write/Tools) + Add-ons group (TeamGQM); the collapse state is binary (expanded 240 px / collapsed 72 px), so a partial state is impossible. |
...
| overflow | Standalone Sider (E4) | ✅ covered | E4 supports a manual collapse via the top chevron (SIDER-02): **240 px → 72 px** icon-only, items shrink to icon + tooltip, never overflowing horizontally. Separately, below a **1024 px** viewport the Standalone view shows an `Alert` (DESIGN_SYSTEM.md §8.2 / spec §17.2) — that width guard is distinct from the manual collapse. |
...
| zero-one-many | Standalone Sider (E4) | ✅ covered | E4 handles 0/1/many Sider items identically — fixed Main group (Chat/Note/Write/Tools) + TeamGQM add-on; the count never varies in Phase 1. |
...
- **Dimension 4 (line 24):** ✅ **Resolved (canonical sync 2026-08-02)** — the Design System row's font description now reads "Inter 600" (was "Inter 600–700"), consistent with the 2-weight cap (400 + 600) in `## Typography`.
