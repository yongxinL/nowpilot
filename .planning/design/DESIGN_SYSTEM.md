# NowPilot — Design System & Visual Language v0.2
...
### 6.4 Display Mode & Theme Packs (user-facing in v0.2)

Two **independent** appearance controls ship in v0.2 (Options → General → Appearance, §8.6):
...
├─────────────────────────────────────────────┤
│ ◈ Auto workflow ▾             📎  🕘  ▢     │  TOOLBAR 44
│ ┌─────────────────────────────────────────┐ │
│ │ Ask anything, choose workflow…      ▷  │ │  INPUT ≥60
│ └─────────────────────────────────────────┘ │
│ Auto · Ready                       ⓘ   ✉   │  STATUS 28
└─────────────────────────────────────────────┘
```
...
- **8.1c Composer toolbar (44 px, above input, space-between):** left = **workflow selector** `◈ Auto workflow ▾` (the only model control here); right = three icon-only actions: **Attach** `📎` · **Chat history** `🕘` · **New chat** `▢`. *(Per mockup, there is no screenshot/snip icon in the composer.)*
...
#
##### 8.1g Workflow selector, not model selector

NowPilot workflows own model requirements. Therefore the chat composer does **not** expose raw model IDs. The left side of the composer toolbar shows a compact Workflow control:

- default: **Auto**;
- examples: Ask, Write, Research, Summarise page, ServiceNow case analysis;
- selecting a workflow changes the workflow contract, allowed tools, context, confirmation behaviour, and capability tier;
- the resolved provider/model is read-only and appears in Activity details or Diagnostics;
- if the workflow is selected by a quick action or slash command, the control updates visibly;
- advanced users configure providers, enabled models, tier mappings, and workflow overrides in Standalone Options.

This avoids conflicting instructions such as selecting a weak model for a workflow that requires tools or a larger context window and keeps cost/fallback policy centrally governed.
...
#
##### 8.6a Example Options information architecture

```text
┌──────────────────────────────────────────────────────────────────────────────┐
│ ‹  Search settings, providers, workflows…              ⌘K        Help   ⋮   │  TOP BAR 56
├──────────────────┬───────────────────────────────────────────────────────────┤
│ (N) NowPilot     │ General                                                   │
│                  │ ┌───────────────────────────────────────────────────────┐ │
│ ● General        │ │ Account                                               │ │
│   Account        │ │ (G) George Li             Local profile      Log out │ │
│   AI access      │ └───────────────────────────────────────────────────────┘ │
│   Appearance     │                                                           │
│                  │ ┌───────────────────────────────────────────────────────┐ │
│ ▤ Notes          │ │ AI access                                             │ │
│   Behaviour      │ │ Providers:  OpenAI ✓  Gemini ✓  Anthropic ○  Ollama ○│ │
│   Backup         │ │ Default workflow             [ Auto            ▾ ]   │ │
│                  │ │ Fast tier                    [ configured model ▾ ]   │ │
│ ⚙ Advanced       │ │ Balanced tier                [ configured model ▾ ]   │ │
│   Providers      │ │ [Manage providers] [Workflow routing] [Test routing] │ │
│   Models         │ └───────────────────────────────────────────────────────┘ │
│   Workflows      │                                                           │
│   MCP servers    │ ┌───────────────────────────────────────────────────────┐ │
│   Memory         │ │ Appearance                                            │ │
│   Diagnostics    │ │ Theme pack [Default ▾]  preview  Display [Auto ▾]    │ │
│   Import/Export  │ │ Language [English (UK) ▾]  Message size [Auto ▾]     │ │
│                  │ └───────────────────────────────────────────────────────┘ │
│ ──────────────── │                                                           │
│ ? Help Centre ↗  │                                 Saved ✓                   │
└──────────────────┴───────────────────────────────────────────────────────────┘
...
- The left menu is grouped, searchable, collapsible, and route-addressable.
- General shows common settings. Advanced exposes provider IDs, model IDs, workflows, MCP, memory, diagnostics and import/export.
- Workflow routing displays workflow → tier → resolved provider/model as a testable chain.
- Every setting validates before persistence and displays Saving, Saved, or Error text.
- Secrets are never echoed after save. A stored API key is represented only as “Configured”.
- Provider/model changes do not silently rewrite workflow definitions. A routing validation result identifies affected workflows before save.
- Destructive actions use a danger zone at the end of the relevant section, not mixed with routine controls.
...
- **Model list — a 6-column table** (per mockup), headed by the count ("N models available") plus **↻ Update list** (refetch from provider, `getModels`) and **+ Add custom model** (define model manually). Columns:
  ...
  **Add custom model** defines model **name + type + context window**. **Only enabled models appear in the workflow/tier resolver**; changes apply in real-time across Side Panel and Standalone views.
- **Footer:** **Cancel** (ghost) / **Save** (primary). Save persists `ProviderConfigSchema` (spec §10.3).
- **How it works (flow):** ① Set up provider (enter API key + optional proxy) → ② Check connection (validate credentials) → ③ Manage models (enable desired models or add custom) → ④ Use in chat (enabled models appear in the workflow selector).
...
Workflow/provider health in the composer status bar uses the same green (healthy) / red (error) dot next to the provider name (§8.1e). Colour is never the only signal — pair with a label/tooltip (§12).
...
- **Status bar:** caption row; workflow name left (turns `colorError` on failure), Help + Feedback right (14 px).
...
| Workflow-selector open | 120 ms fade+slide from anchor |
...
> **Appearance controls (Options → General, both v0.2):** *Display mode* (Auto/Light/Dark) writes to `chrome.storage.sync.np_theme`; *Theme pack* (Default/Liquid Glass/Claude Warm) writes to `chrome.storage.sync.np_theme_pack` (spec §17.1a). Both cascade live to both surfaces via `chrome.storage.onChanged`.
...
*Companion to `PRODUCT_SPEC_v0_2.md`. This document governs visual language only; all functional behaviour is defined by the product specification.*